...
def fetch_nodes_info(shiro_url: str) -> Optional[Dict[str, Any]]:
    ...
    if not shiro_url.startswith(("http://", "https://")):
        ...
        print(f"Warning: URL scheme missing, assuming http://. Using: {shiro_url}", file=sys.stderr)
    ...
    try:
        ...
        print(f"Error: Could not connect to ComfyUI/ShiroUI at {shiro_url}. Is it running?", file=sys.stderr)
